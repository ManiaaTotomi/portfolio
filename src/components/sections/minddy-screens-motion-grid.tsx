...
import {
  MinddyScreenShowcase,
  type MinddySplashTone,
} from "@/components/sections/minddy-screen-showcase";
...
interface MinddyScreensMotionGridProps {
...
export function MinddyScreensMotionGrid({ screens }: MinddyScreensMotionGridProps) {
  ...
  return (
    <div
      className="relative mx-auto mt-24 flex w-full max-w-[1390px] flex-wrap items-start justify-center gap-x-8 gap-y-10 lg:mt-0 lg:flex-nowrap lg:justify-between lg:gap-[69px]"
      ref={containerRef}
    >
      {screens.map((screen) => (
        <div className={screen.desktopOffset} key={screen.src}>
          {screen.isShowcase ? (
            <MinddyScreenShowcase
              ...
              splashTone={screen.splashTone ?? "violetForest"}
              src={screen.src}
              width={screen.width}
            />
          ) : (
